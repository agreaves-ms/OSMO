...
| Feature        | Regular Workflows                               | Apps                                                        |
|----------------|-------------------------------------------------|-------------------------------------------------------------|
| **Storage**    | Local YAML files on your machine                | Centralized in OSMO cloud                                   |
| **Sharing**    | Manual (email, chat, shared folders)            | Automatic discovery with<br/><br/>`osmo app list`           |
| **Parameters** | Hidden in YAML, users must read docs            | Self-documenting with<br/><br/>`osmo app show`              |
| **Updates**    | Re-share files to everyone                      | Automatic - everyone uses latest version                    |
| **Discovery**  | Ask colleagues “Do we have a workflow for X?”   | Search and browse:<br/><br/>`osmo app list --name training` |
| **Versioning** | Manual file naming (v1, v2, final, final-final) | Built-in version control                                    |
...
#### SEE ALSO
See [here](specification/templates_and_tokens.md#workflow-spec-templates-and-special-tokens) for more details on template variables and special tokens.
...
Instead, setup a [credential](specification/secrets.md#workflow-spec-secrets) and use it in the app spec:
...
3. The app is not currently in a `PENDING` or `DELETED` state

## CLI Reference

#### SEE ALSO
See [here](../appendix/cli/cli_app.md#cli-reference-app) for a complete CLI reference for working with apps.
