...
| **Field**       | **Description**                                                                                                                                                                                                                     |
|-----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `exec_timeout`  | Maximum execution time for a workflow before the service cleans it up. This is the time since<br/>the **workflow status** is set to `READY`.                                                                                        |
| `queue_timeout` | Maximum queue time for a workflow in the workflow queue before the service cleans it up.<br/>This is the time calculated after the **workflow status** is set to `PENDING` and until<br/>the **workflow status** is set to `READY`. |
